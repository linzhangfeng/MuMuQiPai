...
def main():
    ...
    jsb_root = os.path.abspath(os.path.join(project_root, 'cocos2d/cocos/scripting/js-bindings'))
    cxx_generator_root = os.path.abspath(os.path.join(project_root, 'cocos2d/tools/bindings-generator'))
    ...
    config.set('DEFAULT', 'projectroot', project_root)
    ...
    try:
        ...
        tojs_root = '%s/mmqpjsbind' % project_root
        output_dir = '%s/mmqpjsbind' % class_root

        cmd_args = {
            'test.ini': ('lin', 'js_lin_auto')
        }
        target = 'spidermonkey'
        generator_py = '%s/generator.py' % cxx_generator_root
        print "generator="+generator_py
        for key in cmd_args.keys():
            args = cmd_args[key]
            cfg = '%s/%s' % (tojs_root, key)
            usercfg = '%s/%s' % (tojs_root, 'userconf.ini')
            print 'Generating args for %s...' ,args
