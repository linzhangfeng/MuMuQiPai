...
def main():
    ...
    jsbind_root = os.path.abspath(os.path.join(project_root, 'common', 'CommonJsBindTest'))
    classes_root = os.path.abspath(os.path.join(project_root, 'common', 'CommonLib'))
    ...
    config.set('DEFAULT', 'jsbindroot', jsbind_root)
    config.set('DEFAULT', 'classesroot', classes_root)
    ...
    print 'generating userconf.ini...%s' % conf_ini_file
    ...
    try:
        ...
        tojs_root = '%s/common/CommonLib' % project_root
        output_dir = '%s/common/CommonLib/jsbind' % project_root

        cmd_args = {
            'common.ini': ('common', 'js_common_auto')
        }
        target = 'spidermonkey'
        generator_py = '%s/generator.py' % cxx_generator_root
        for key in cmd_args.keys():
            args = cmd_args[key]
            cfg = '%s/%s' % (jsbind_root, key)
            usercfg = '%s/%s' % (jsbind_root, 'userconf.ini')
            print 'Generating bindings for %s...' % (key[:-4])
            command = '%s %s %s -s %s -t %s -o %s -n %s %s' % (python_bin, generator_py, cfg, args[0], target, output_dir, args[1], usercfg)
            _run_cmd(command)
