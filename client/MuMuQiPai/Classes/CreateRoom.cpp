...
//            this->removeFromParent();
//        }
//    }, "gameClasli/recommendConfig_v3", jsonDate.toStyledString(), "createSet");
...
//    mbg->addChild(closeBtn,10);
    ...
    //添加这段代码
    if (!setData[baoIndex]["gameRound"].isNull()){
        Json::Value value_date;
        Json::Reader read;
        read.parse(setData[baoIndex]["gameRound"].asString(), value_date);
        timeArray = value_date;
        printf("EEEE = %s\n", timeArray.toStyledString().c_str());
        time = timeArray[countLastIndex]["round"].asInt();
    }
    ...
    CallFunc* func1 = CallFunc::create([=]
                                       {
                                           ...
                                           list_1->setDirection(ui::ScrollView::Direction::VERTICAL);
...
void CreateRoom::initTextData()
{
    ...
    for(int i = 0; i < 2;i++){
        ...
        for(int k = 0 ; k < 3; k++){
            ...
            gameCustomPlayValInfoList["playId"] = 1132001;
            gameCustomPlayValInfoList["playName"] = "抓2马";
    ...
    m_showData = data;
    log("lin=initTextData data [%s]", data.toStyledString().c_str());
}
